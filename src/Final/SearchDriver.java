...
public class SearchDriver {
    ...
    private void setupPrompt() throws IOException
    {
...
for (int i=0; i < allTweets.size(); ++i)
{
    ...
    for (int j=0; j< results.size(); ++j)
    {
        ...
        {associationDisplay.put(j, i);
            isduplicate = true;}
    }
    if (!isduplicate)
    {
        results.add(allTweets.get(i));
        associationDisplay.put(results_index, i);
        results_index++;
    }
}
return results;
...
void showRankedTweets()
    ...
    {	
            ...
            for (int i = 0; i<display_items; i++)
            {
                knowing.updateWeights(collected.get(displayAssociation.get(i)));
            }
    }
}
